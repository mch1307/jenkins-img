...
function prop {
    grep "${1}" /dev/shm/gvs|cut -d'=' -f2
}
UCPUSER=$(prop 'UCPUSER')
UCPPWD=$(prop 'UCPPWD')

mkdir -p /dev/shm/jks.init
cd  /dev/shm/jks.init
echo curl -sk -d '{"username":"'$UCPUSER'","password":"'${UCPPWD}'"}' https://${UCPURL}/auth/login | awk -F '[":]' '{print $5}'
